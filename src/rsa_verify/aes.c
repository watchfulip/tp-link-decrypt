...
#define TD(a,b,c,d) Td0[(a >> 24)] ^ Td1[(b >> 16) & 0xff] ^ Td2[(c >> 8) & 0xff] ^ Td3[(d) & 0xff]
#define TE(a,b,c,d) Te0[(a >> 24)] ^ Te1[(b >> 16) & 0xff] ^ Te2[(c >> 8) & 0xff] ^ Te3[(d) & 0xff]
#define TE4(a,b,c,d) (Te4[(a >> 24)] & 0xff000000) ^ (Te4[(b >> 16) & 0xff] & 0x00ff0000) ^ (Te4[(c >> 8) & 0xff] & 0x0000ff00) ^ (Te4[(d) & 0xff] & 0x000000ff)
#define TD4(a,b,c,d) (Td4[(a >> 24)] & 0xff000000) ^ (Td4[(b >> 16) & 0xff] & 0x00ff0000) ^ (Td4[(c >> 8) & 0xff] & 0x0000ff00) ^ (Td4[(d) & 0xff] & 0x000000ff)
#define TDE(a) Td0[Te4[(a >> 24)] & 0xff] ^ Td1[Te4[(a >> 16) & 0xff] & 0xff] ^ Td2[Te4[(a >> 8) & 0xff] & 0xff] ^ Td3[Te4[(a) & 0xff] & 0xff]
#define TE4R(t,i) (Te4[(t >> 16) & 0xff] & 0xff000000) ^ (Te4[(t >> 8) & 0xff] & 0x00ff0000) ^ (Te4[(t) & 0xff] & 0x0000ff00) ^ (Te4[(t >> 24)] & 0x000000ff) ^ rcon[i]
...
int AES_set_encrypt_key(const unsigned char *key, const int bits, struct AES_key *aeskey)
{
	...
	if (bits == 256) {
		for (;;) {
			...
			temp = rk[11];
			rk[12] = rk[4] ^ TE4(temp, temp, temp, temp);
			rk[13] = rk[5] ^ rk[12];
			rk[14] = rk[6] ^ rk[13];
			rk[15] = rk[7] ^ rk[14];
...
int AES_encrypt(const unsigned char *in, unsigned char *out, const struct AES_key *key)
{
	...
	for (;;) {
		t0 = TE(s0, s1, s2, s3) ^ rk[4];
		t1 = TE(s1, s2, s3, s0) ^ rk[5];
		t2 = TE(s2, s3, s0, s1) ^ rk[6];
		t3 = TE(s3, s0, s1, s2) ^ rk[7];

		rk += 8;
		if (--r == 0) {
			break;
		}
		s0 = TE(t0, t1, t2, t3) ^ rk[0];
		s1 = TE(t1, t2, t3, t0) ^ rk[1];
		s2 = TE(t2, t3, t0, t1) ^ rk[2];
		s3 = TE(t3, t0, t1, t2) ^ rk[3];
	}

	s0 = TE4(t0, t1, t2, t3) ^ rk[0];
	PUTU32(out, s0);
	s1 = TE4(t1, t2, t3, t0) ^ rk[1];
	PUTU32(out + 4, s1);
	s2 = TE4(t2, t3, t0, t1) ^ rk[2];
	PUTU32(out + 8, s2);
	s3 = TE4(t3, t0, t1, t2) ^ rk[3];
	PUTU32(out + 12, s3);
...
int AES_decrypt(const unsigned char *in, unsigned char *out, const struct AES_key *key)
{
	...
	for (;;) {
		t0 = TD(s0, s3, s2, s1) ^ rk[4];
		t1 = TD(s1, s0, s3, s2) ^ rk[5];
		t2 = TD(s2, s1, s0, s3) ^ rk[6];
		t3 = TD(s3, s2, s1, s0) ^ rk[7];

		rk += 8;
		if (--r == 0) {
			break;
		}
		s0 = TD(t0, t3, t2, t1) ^ rk[0];
		s1 = TD(t1, t0, t3, t2) ^ rk[1];
		s2 = TD(t2, t1, t0, t3) ^ rk[2];
		s3 = TD(t3, t2, t1, t0) ^ rk[3];
	}

	s0 = TD4(t0, t3, t2, t1) ^ rk[0];
	PUTU32(out, s0);
	s1 = TD4(t1, t0, t3, t2) ^ rk[1];
	PUTU32(out + 4, s1);
	s2 = TD4(t2, t1, t0, t3) ^ rk[2];
	PUTU32(out + 8, s2);
	s3 = TD4(t3, t2, t1, t0) ^ rk[3];
	PUTU32(out + 12, s3);
...
int AES_cbc_encrypt(const unsigned char *in, unsigned char *out,
	unsigned int len, const struct AES_key *key, unsigned char *iv, int forward)
{
	...
	if (forward) {
		while (len >= sizeof(tmp)) {
			for (i = 0; i < sizeof(tmp); i++) tmp[i] = in[i] ^ iv[i];
			AES_encrypt(tmp, out, key);
			memcpy(iv, out, sizeof(tmp));
			len -= sizeof(tmp);
			in += sizeof(tmp);
			out += sizeof(tmp);
		}
		if (len) {
			for (i = 0; i < len; i++)tmp[i] = in[i] ^ iv[i];
			for (i = len; i < sizeof(tmp); i++) tmp[i] = iv[i];
			AES_encrypt(tmp, out, key);
			memcpy(iv, out, sizeof(tmp));
		}
	}
		...
		while (len >= sizeof(tmp)) {
			...
			for (i = 0; i < sizeof(tmp); i++) out[i] ^= iv[i];
			memcpy(iv, tmp, sizeof(tmp));
			len -= sizeof(tmp);
			in += sizeof(tmp);
			out += sizeof(tmp);
		}
